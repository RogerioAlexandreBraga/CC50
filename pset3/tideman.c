    ...
    for (int i = 0; i < pair_count; i++)
    {
        ...
        for (int j = 0; j < pair_count; j++)
        {
            ...
            {                 // Atribuindo o iswinner como false e recomeçando tudo só que com i sendo = 1
                iswinner = false; // Com i é igual a 1 ele vai testado e acha em locked[0][1] verdadeiro, quebrando o for e
                                  // começando de novo
        ...
        if (iswinner ==
            true) // iscyrcle já que a vitoria de charlie sobre alice veio primeiro, pois foi maior do que a de bob x charlie
        {
            printf("%s\n", candidates[i]);
        }
    }
